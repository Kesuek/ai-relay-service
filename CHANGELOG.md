...
- **T-109 Observability — Metrics + eingebautes Dashboard + strukturierte Logs (2026-08-04)**: Der Relay wird observierbar. (1) Neues Modul `core/metrics.py` bündelt In-Process-Counter (Auth-Failures via Middleware) und DB-abgeleitete Gauges (Tasks/Stages/Nodes/Queue) und rendert `/metrics` im Prometheus-Textformat. (2) `/metrics` (offen wie `/health`, kein Secrets im Output) und `/ready` (Liveness für die Task-Verarbeitung — prüft DB, Scheduler/Maintenance-Loop und Event-Bus) in `main.py`. (3) Auth-Failure-Counter via globale Middleware zählt 401/403/429 ohne `auth.py` anzufassen. (4) JSON-Metrik-API `/relay/v2/dashboard/api/metrics` (Session-Auth) + eingebautes HTML-Metrics-Dashboard `/relay/v2/dashboard/metrics` mit Zahl-Karten und minimalen Balkendiagrammen (CSP-konform, externes `metrics.js`). Das Metrics-Dashboard ist aus dem Admin-Dashboard (Topnav) und dem Community-Portal (Button) verlinkt. (5) Strukturierte JSON-Logs via Standard-Logging-Formatter (`core/logging_setup.py`), eine Middleware setzt pro Request eine `trace_id` (16-hex) via `contextvars` und gibt sie als `X-Relay-Trace-Id`-Header zurück — jede Log-Zeile eines Requests trägt dieselbe `trace_id`. Kein neues Dependency (kein structlog, kein Prometheus-Server/Grafana).
