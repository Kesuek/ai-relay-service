...
logger = logging.getLogger("vault_node")
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Vault capability node for AI-Relay-Service v2")
...
def _do_vault_work(stage: Dict[str, Any]) -> Dict[str, Any]:
    """Minimal simulated vault work."""
    payload = stage.get("payload") or {}
    logger.info("Doing vault work for stage %s", stage.get("stage_id"))
    time.sleep(0.5)
    return {
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        logger.info("Starting vault node %s work loops", args.node_id)
